...
namespace ImViewLite.Helpers
{
    public static class ClipboardHelper
    {
        ...
        public static bool CopyImage(Image img)
        {
            try
            {
                if (InternalSettings.Use_Alternate_Copy_Method)
                {
                    return CopyImageAlternative(img);
                }

                if (InternalSettings.Replace_Transparency_On_Copy)
                {
                    return CopyImageDefaultFillBackground(img, InternalSettings.Fill_Transparency_On_Copy_Color);
                }
        ...
        {
            using (Bitmap bmpNonTransparent = new Bitmap(img.Width, img.Height,PixelFormat.Format24bppRgb))
        ...
        public static Bitmap GetImage()
        {
            ...
            try
            {
                ...
                object data = Clipboard.GetData(FORMAT_PNG);
                ...
                if (stream != null)
                {
                    Bitmap result = Image.FromStream(stream).Copy();
    ...
    // Credits: https://github.com/ShareX/ShareX/blob/master/ShareX.HelpersLib/Helpers/ClipboardHelpersEx.cs
